...
1. **Research & Solution Design**: 
   ...
   - Structure your handoff as JSON matching the `DispatcherHandoff` model in `tools/handoff/models.py`.
   - Invoke `python3 tools/handoff/generate.py --agent dispatcher --json-data '{"technical_specification": "...", "module_map": "...", "next_steps": "..."}'` passing your structured JSON string.
   - Wait for the script to validate and save the JSON handoff file, then run `git add docs/issues/` and `git commit -m "docs: add dispatcher handoff"`.
