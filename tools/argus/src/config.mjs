...
export const PERSIST_ROOT = '.athena-telemetry';
...
export function resolveConfig(flags = {}, env = process.env) {
  ...
  const explicitPersist = asDir(flags.persist ?? env.ATHENA_OBS_PERSIST);
  ...
  const config = {
    ...
    retentionMs: open
      ? Infinity
      : parseDuration(flags.retention ?? env.ATHENA_OBS_RETENTION, 24 * 60 * 60 * 1000),
