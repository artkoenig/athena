...
- `store.test.mjs` — `TelemetryStore` in memory: token/cost rollups, temporality, spans, tool results, todos/tasks, the ring-buffer eviction that bounds every index, and (`/* --- content --- */`) `queryContent`/`getContextAt`, the `maxContentBytes` budget over every content-bearing text (prompts, assistant responses, tool details, tool-content span events, and request/response bodies), and the agent-lane attribution walk from a body event's `spanId` up the span tree to its dispatching `Agent` execution span.
...
- payload builders — `tracePayload`/`logsPayloadJson` (server), `attrs` (decode), `metric`/`log`/`span`/`bodyEvent` (store, persist): literals shaped like the wire or like decoder output. `bodyEvent(eventName, {model, body, truncated, querySource}, timeMs, spanId)` builds a `claude_code.api_request_body`/`claude_code.api_response_body` log record in the attribute shape real captured telemetry carries (`body_length`/`body_truncated` as strings); `log`'s fourth argument is the record's `spanId`, for attribution-through-the-span-tree cases. `span`'s `extra.events` carries caller-supplied span events (default `[]`); `toolOutputEvent(timeMs, text)` (store) builds one in the decoder's shape, `{ timeMs, name: 'tool.output', attrs: { 'tool.output': text } }` — the attribute key is arbitrary and never asserted by name, since the store counts every string value on a span event. `logsPayloadBody`/`agentDispatchTracePayload` (server) are the OTLP-wire equivalents — a log record carrying a `spanId`, and the `claude_code.tool`(`tool_name: 'Agent'`)/`claude_code.tool.execution` span pair a subagent dispatch produces.
