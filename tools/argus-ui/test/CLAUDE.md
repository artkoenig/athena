...
- `run.test.mjs` — the pure functions of `public/run.js`: `isClosedIncrement`/`CLOSED_STATUSES`, `incrementCounts`, `pickRunId`, `runFrame`, `shouldLoadRun`, `runningView`, `childEntries`, `listRefs`, `badgeOf`, `hintOf`, `openPathsFor`, and the `renderRunList`/`renderRun`/`renderTree`/`renderNode`/`renderRunning` markup — including the tree mirroring the recorded document key for key, list order and index labels, what arrives open and what arrives folded, a page of text folding into a `<pre>` of its own, every JSON type rendered as itself, the unique `data-panel` path per disclosure and its survival of a re-cut that moved the row, and the step in flight with its open prompt.
- `page.test.mjs` — `public/app.js`, `index.html`, `styles.css` and the `.md` pages checked as source text: imports, event wiring, the boot path and the run view's fetch-and-repaint wiring, state shape, deleted views staying deleted, `refreshRuns` delegating its re-fetch decision to `shouldLoadRun`, a repaint leaving the reader inside the block they were reading, and a run repaint leaving them on the node, the scroll position and the focus they had. `app.js` touches browser globals, so it is never imported.
