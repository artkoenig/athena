...
- `run.test.mjs` — the pure functions of `public/run.js`: `isClosedIncrement`/`CLOSED_STATUSES`, `incrementCounts`, `pickRunId`, `runFrame`, `shouldLoadRun`, `stepView`, `STEP_SHED_NOTE`, and the `renderRunList`/`renderRun`/`renderSteps` markup.
