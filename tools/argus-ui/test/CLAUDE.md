...
- `run.test.mjs` — the pure functions of `public/run.js`: `isClosedIncrement`/`CLOSED_STATUSES`, `incrementCounts`, `pickRunId`, `runFrame`, `shouldLoadRun`, `runningView`, `childEntries`, `badgeOf`, `hintOf`, `openPathsFor`, and the `renderRunList`/`renderRun`/`renderTree`/`renderNode`/`renderRunning` markup — including the tree mirroring the recorded document key for key, list order and index keys, what arrives open and what arrives folded, a page of text folding into a `<pre>` of its own, every JSON type rendered as itself, the unique `data-panel` path per disclosure, and the step in flight with its open prompt.
